									...
									<TD>
										<TABLE id=grid
											...
											<TBODY>
                                                ...
                                                <%--放到域对象中使用jstl foreach标签取 ,当时用值栈时,再使用jstl这么取值 性能低--%>
												<c:forEach items="${pageBean.list}" var="customer">
												<TR
													style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
													<TD>${customer.custName }</TD>
													<TD>${customer.dataDictionary.level}</TD>
								...
								<TR>
									<TD><SPAN id=pagelink>
											<DIV
												...
												<%--
													前一页 当前页 -1 判断如果是第一页，没有前一页
													后一页 当前页 +1 判断如果是最后一页，没有后一页
												--%>
												<c:if test="${pageBean.currentPage != 1}">
												[<A href="${pageContext.request.contextPath}
														/customer_listPage.action?currentPage=${pageBean.currentPage - 1}">前一页</A>]
												</c:if>

												<c:if test="${pageBean.currentPage != pageBean.totalPage}">
												[<A href="${pageContext.request.contextPath}
														/customer_listPage.action?currentPage=${pageBean.currentPage + 1}">后一页</A>]
												</c:if>
